...
# max value in signal will be = strength => max divergence from pxl value will be strength/255
...
def create_noise(shape, intensity, fraction, epsilon):
    ...
    if intensity / 255 > epsilon:
        intensity = int(epsilon * 255)
...
def find_edges(image, intensity, threshold, epsilon):
    ...
    for i in range(image.shape[0]):
        for j in range(image.shape[1]):
            ...
            # considered an edge
            if edges[i, j] > int(threshold * 255):
...
def create_patch(shape, size, epsilon):
    ...
    for i in range(size // 2):
        for j in range(size // 2):
            patch[i*2, j*2] = np.floor(epsilon * 255)
        for j in range(size // 2):
            patch[i*2+1, j*2+1] = np.floor(epsilon * 255)
...
def apply_noise(image, signal):
    noisy = image.astype(np.float32) + signal
    noisy = np.rint(noisy)
